...
$SCSS         = $root . '/public/assets/scss/' . $template . '.scss';
$CSS          = $root . '/public/assets/css/'  . $template . '.css';
...
if ($template == 'default' or !file_exists($SCSS)) {
	$SCSS         = $root . '/public/assets/scss/default.scss';
	$CSS          = $root . '/public/assets/css/default.css';
	$CSSKirbyPath = 'assets/css/default.css';
}
// If the CSS file doesn't exist create it so we can write to it
if (!file_exists($CSS)) {
	if (!file_exists($root . '/public/assets/css/')) {
	    mkdir($root . '/public/assets/css/', 0755, true);
	}
...
if ($kirby->option('scssNestedCheck')) {
	$SCSSDirectory = $root . '/public/assets/scss/';
...
if (!file_exists($CSS) or $SCSSFileTime > $CSSFileTime ) {
    ...
    $importPath = $root . '/public/assets/scss';
